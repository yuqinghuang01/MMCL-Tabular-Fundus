...
def load_datasets(hparams):
  if hparams.datatype == 'imaging':
    ...
    train_dataset = ContrastiveGraphAndTabularDataset(
      hparams.data_train_eval_graph, hparams.data_train_eval_tabular, hparams.corruption_rate,
      hparams.field_lengths_tabular, hparams.one_hot, hparams.labels_train_eval_graph)
    val_dataset = ContrastiveGraphAndTabularDataset(
      hparams.data_val_eval_graph, hparams.data_val_eval_tabular, hparams.corruption_rate, 
      hparams.field_lengths_tabular, hparams.one_hot, hparams.labels_val_eval_graph)
    hparams.input_size = train_dataset.get_input_size()
...
def evaluate(hparams, wandb_logger):
  ...
  wandb_logger.log_metrics({f'best.val.{hparams.eval_metric}.acc': model.best_val_acc})
  wandb_logger.log_metrics({f'best.val.{hparams.eval_metric}.pre': model.best_val_pre})
  wandb_logger.log_metrics({f'best.val.{hparams.eval_metric}.rec': model.best_val_rec})
  wandb_logger.log_metrics({f'best.val.{hparams.eval_metric}.f1': model.best_val_f1})
