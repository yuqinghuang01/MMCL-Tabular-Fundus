...
from models.MultimodalGraph import MultimodalGraph
...
def load_datasets(hparams):
  if hparams.datatype == 'imaging_tabular':
    ...
    train_dataset = ContrastiveGraphAndTabularDataset(
      hparams.data_train_graph, hparams.data_train_tabular, hparams.corruption_rate,
      hparams.field_lengths_tabular, hparams.one_hot, hparams.labels_train)
    val_dataset = ContrastiveGraphAndTabularDataset(
      hparams.data_val_graph, hparams.data_val_tabular, hparams.corruption_rate, 
      hparams.field_lengths_tabular, hparams.one_hot, hparams.labels_val)
    hparams.input_size = train_dataset.get_input_size()
...
def pretrain(hparams, wandb_logger):
  ...
  if hparams.datatype.startswith('graph'):
    ...
    #print(batch) # MyDataBatch(x=[7223, 3], edge_index=[2, 7588], edge_attr=[7588, 14], tabular=[64, 89], corrupted_tabular=[64, 89], label=[64], batch=[7223], ptr=[65])
  ...
  if hparams.online_mlp:
    ...
    callbacks.append(SSLOnlineEvaluator(z_dim = model.pooled_dim, batch_size = hparams.batch_size, multimodal = hparams.datatype, hidden_dim = hparams.embedding_dim, num_classes = hparams.num_classes, swav = False))
