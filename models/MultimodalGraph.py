...
class MultimodalGraph(Pretraining):
  """
  ...
  def __init__(self, hparams):
    ...
    # Graph
    self.initialize_graph_encoder_and_projector()
    
    if self.hparams.graph_pretrain_checkpoint:
      self.load_pretrained_graph_weights()
    ...
    print(f'Graph model, multimodal: {self.encoder_graph}\n{self.projector_graph}')
  ...
  def training_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor]], _) -> torch.Tensor:
    ...
    # Augmented views
    z0, embeddings = self.forward_graph(data)
    z1, _ = self.forward_tabular(data.corrupted_tabular)
    loss, logits, labels = self.criterion_train(z0, z1, data.label)
    
    self.log(f"multimodal.train.loss", loss, on_epoch=True, on_step=False, batch_size=self.batch_size)
    if len(data)==self.hparams.batch_size:
      self.calc_and_log_train_embedding_acc(logits=logits, labels=labels, modality='multimodal')

    return {'loss':loss, 'embeddings': embeddings, 'labels': data.label}
  ...
  def validation_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor]], _) -> torch.Tensor:
    ...
    # Unaugmented views
    z0, embeddings = self.forward_graph(data)
    z1, _ = self.forward_tabular(data.tabular)
    loss, logits, labels = self.criterion_val(z0, z1, data.label)

    self.log("multimodal.val.loss", loss, on_epoch=True, on_step=False, batch_size=self.batch_size)
    if len(data)==self.hparams.batch_size:
      self.calc_and_log_val_embedding_acc(logits=logits, labels=labels, modality='multimodal')

    return {'embeddings': embeddings, 'labels': data.label}
  ...
  def configure_optimizers(self) -> Tuple[Dict, Dict]:
    ...
    optimizer = torch.optim.Adam(
      [
        {'params': self.encoder_graph.parameters()}, 
        {'params': self.projector_graph.parameters()},
