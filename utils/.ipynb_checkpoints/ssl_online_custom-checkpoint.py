...
class SSLOnlineEvaluator(Callback):  # pragma: no cover
    ...
    def to_device(self, batch: Sequence, device: Union[str, torch.device]) -> Tuple[Tensor, Tensor]:
        ...
        if self.swav:
            x, y = batch
            x = x[0]
        elif self.multimodal == "imaging_tabular":
            x_i, x_t, y, x_orig = batch
            x = x_orig
        elif self.multimodal == "graph_tabular":
            x = batch
            y = batch.label
        else:
            _, x, y = batch
    ...
    def on_train_batch_end(
        ...
        pl_module.log("classifier.train.loss", mlp_loss, on_step=False, on_epoch=True, sync_dist=True, batch_size=self.batch_size)
        pl_module.log("classifier.train.auc", train_auc, on_step=False, on_epoch=True, sync_dist=True, batch_size=self.batch_size)
        pl_module.log("classifier.train.acc", train_acc, on_step=False, on_epoch=True, sync_dist=True, batch_size=self.batch_size)
    ...
    def on_validation_batch_end(
        ...
        pl_module.log("classifier.val.loss", mlp_loss, on_step=False, on_epoch=True, sync_dist=True, batch_size=self.batch_size)
        pl_module.log("classifier.val.auc", val_auc, on_step=False, on_epoch=True, sync_dist=True, batch_size=self.batch_size)
        pl_module.log("classifier.val.acc", val_acc, on_step=False, on_epoch=True, sync_dist=True, batch_size=self.batch_size)
